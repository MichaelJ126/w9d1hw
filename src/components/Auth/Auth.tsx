    ...
    const signIn = async () => {
        ...
        onAuthStateChanged(auth, (user) => {
            ...
            if (user) {
                ...
                setMessage(`You have successfully logged in ${user.email}`)
...
const SignUp = () => {
    ...
    return (
        <Box>
            <form onSubmit = {handleSubmit(onSubmit)}>
                <Typography variant='h6'>Free Membership</Typography>
                <Box>
                    <label htmlFor='email'></label>
                    <InputText {...register('email')} name='email' placeholder='Enter Email' />
                    <label htmlFor='password'></label>
                    <InputPassword {...register('password')} name='password' placeholder='Password must be 6 characters or longer' />
                </Box>
                <Button type='submit'>Submit</Button>
